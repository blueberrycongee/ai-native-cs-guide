...
任何一次“完善仓库”的任务，都必须先检查本节。最终交付必须让读者看清一个方向的边界、信息来源、代表项目、关键概念和工程问题。缺少这些内容时，不要说仓库已经完成。

每条重点方向至少要写清楚这些内容：

- 方向总览：说明这个方向讨论什么、不讨论什么，和其他方向的边界在哪里。
- 信息来源：整理官方文档、论文、release note、技术博客、issue、discussion 等入口，并说明这些入口对应哪些问题。
- 代表项目：列出真实项目或代码库，说明它代表哪种系统形态，以及和哪些概念、组件或工程问题相关。
- 工具生态：按问题域整理框架、库、平台和服务，不写成工具学习路线。
- 概念文章：把具体机制、工程边界和失败模式讲清楚，并用内部链接连接到相关项目和资料。

方向文档如果只有“是什么”“为什么重要”“推荐资料”，还不够。它至少要能回答：

- 这个方向有哪些核心问题。
- 这些问题在真实系统里通常落在哪些组件上。
- 哪些开源项目体现了这些系统形态。
- 哪些资料是一手入口，哪些只适合当线索。
- 仓库里已有或后续会写哪些文章连接这些主题。

这些要求优先级高于目录结构。目录可以调整，但不能用“结构已经有了”替代内容完成。

### 方向目录完整性要求

每条重点方向不能只有一个总览页。总览页只负责说明范围和边界，不能替代概念、项目、信息来源和工具框架说明。
...
- 方向首页：说明这个方向解决什么问题，和其他方向的边界是什么。
- 概念地图或概念目录：列出这个方向内部反复出现的核心概念，并链接到文章页。不要只列名词，要说明概念之间如何连接。
- 信息来源页：列出官方文档、论文、release note、技术博客、issue、discussion、社区讨论等信息源，并说明它们对应哪些问题。
- 开源项目页：列出真实项目或代码库，说明它代表哪类系统形态，并链接到相关概念或文章。
- 框架和工具页：按问题域整理框架、库、平台、工具或服务，不写成“必须会 / 只需知道”的学习分层。
- 项目页或工程文章：围绕具体工程问题展开，不写成通用作业清单。

如果某个方向暂时没有足够内容，宁可标成未完成，也不要只留下一个总览页然后说这个方向已经完成。
...
这些文档要互相链接。方向首页链接信息来源、开源项目、框架工具和核心概念；项目页链接概念和外部项目；概念页链接项目和资料入口。不要形成孤立页面。
...
- 不把所有相关概念都塞进一个方向目录
- 不写传统培训班式路线图
- 方向总览页只说明讨论范围、核心问题和目录结构，不写学习顺序、必做项目、必学知识或阶段验收标准
- 不把“看过资料”伪装成“掌握能力”
- 不推荐没有判断标准的资源清单
- 不把 demo 当成真实项目
- 不写无法指导读者做选择的解释
- 不让文档过度代码化。概念文档不是 API reference，也不是字段说明书
- 信息来源页只做推荐和简短说明，不写成信息素养教程。不分“一级来源 / 二级来源”，不教读者如何判断来源好坏，不加入“少看什么”“不要被带跑”这类指导性段落。默认读者有判断力；仓库负责筛选入口和说明推荐理由。
- 开源项目页只列项目入口、系统形态和相关主题，不写成学习路径。不分“必看 / 扩展”，不指导读者该先看哪个项目。
- 如果解释一个概念需要列很多字段、参数、配置项或实现细节，优先链接外部文档、论文、GitHub 仓库或代码示例，不在仓库里重写
- 仓库可以解释资料之间的关系、背景和边界，但不替读者安排阅读顺序，也不复制别人的完整技术说明
